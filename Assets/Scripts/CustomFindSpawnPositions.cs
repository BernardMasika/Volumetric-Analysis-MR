...
public class CustomFindSpawnPositions : MonoBehaviour
{
...
/*#if UNITY_EDITOR
    ...
    public void StartSpawn(MRUKRoom room)
    {
        ...
        foreach (var anchor in anchors)
        {
            ...
            if (anchor.AnchorLabels[0].Equals(MRUKAnchor.SceneLabels.TABLE.ToString()))
            {
                _anchor = anchor;
            }
        ...
        for (int i = 0; i < SpawnAmount; ++i)
        {
            for (int j = 0; j < MaxIterations; ++j)
            {
                ...
                spawnNormal = _anchor.transform.forward;
                spawnPosition = _anchor.transform.position;
                /*var center = spawnPosition + spawnNormal * centerOffset;
                // Ensure the center of the prefab will not spawn inside a scene volume
                if (room.IsPositionInSceneVolume(center))
                {
                    continue;
